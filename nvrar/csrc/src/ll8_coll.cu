...
__global__ void pack_payloads_kernel(Payload8B<T>* __restrict__ payloads,
                                     ...
                                     size_t num_elements, uint32_t* seq_num) {
...
template <typename T>
__global__ void unpack_payloads_kernel(
    T* __restrict__ data, const Payload8B<T>* __restrict__ payloads,
    size_t num_elements, uint32_t* seq_num, uint64_t* seq_num_signal, int steps,
    int steps_intra, int steps_inter) {
  int mype = nvshmem_my_pe();
  int npes = nvshmem_n_pes();

  uint64_t local_seq_num = *seq_num;

  // This is okay here because we are copying from the source buffer and so the
  // recv buffer can be overwritten
  if (threadIdx.x == 0 && blockIdx.x == 0) {
    int partner;
    for (int step = steps_intra; step < steps; ++step) {
      int inter_node_step = step - steps_intra;
      partner = mype ^ (1 << step);
      nvshmem_uint64_atomic_set(seq_num_signal + inter_node_step,
                                local_seq_num + 1, partner);
    }
  }
...
template <typename T>
__device__ void __forceinline__ process_chunk_warp_all_ready(
    int lane, int warp, int warps, Payload8B<T>* __restrict__ acc_chunk,
    const Payload8B<T>* __restrict__ dst_chunk,
    const Payload8B<T>* __restrict__ src_chunk, size_t chunk_size,
    uint32_t seq_num_local, uint64_t timeout_ns) {
  for (size_t tile = warp * 32; tile < chunk_size; tile += warps * 32) {
    ...
    const volatile uint32_t* flag_ptr =
        valid ? &dst_cell->flag : (const volatile uint32_t*)NULL;

    // Cooperative spin until *all valid lanes* see their flag == seq
    const uint64_t start = read_globaltimer();
    const uint64_t deadline = start + timeout_ns;  // already in ns
    ...
    while (true) {
      ...
      if (lane == 0 && read_globaltimer() >= deadline) {
        ...
        printf("[pe %d][block %d][thread %d] timeout seq_num=%u got=%u\n",
               (int)nvshmem_my_pe(), (int)blockIdx.x, (int)threadIdx.x,
               (int)seq_num_local, (int)dst_cell->flag);
...
template <typename T>
__global__ void recursive_allreduce_kernel_payload(
    Payload8B<T>* dst, Payload8B<T>* src, size_t nreduce, int steps,
    int steps_intra, int steps_inter, size_t chunk_elems, size_t stride_size,
    uint32_t* seq_num, uint64_t* seq_num_signal) {
  int mype = nvshmem_my_pe();
  int npes = nvshmem_n_pes();
  ...
  const size_t total_cells =
      (nreduce + ELEMS_PER_PAYLOAD - 1) / ELEMS_PER_PAYLOAD;
  ...
  size_t chunk_cells = max(
      (size_t)32, (chunk_elems + ELEMS_PER_PAYLOAD - 1) / ELEMS_PER_PAYLOAD);
  ...
  for (int step = steps_intra; step < steps; ++step) {
    ...
    for (size_t chunk_idx = 0; chunk_idx < num_chunks; ++chunk_idx) {
      const size_t chunk_size =
          min(chunk_cells, cells_this_block - chunk_idx * chunk_cells);
      src_chunk = src + offset + chunk_idx * chunk_cells;
      dst_chunk =
          reinterpret_cast<Payload8B<T>*>(reinterpret_cast<uint8_t*>(dst) +
                                          (inter_node_step + 1) * stride_size) +
      ...
      nvshmemx_put64_nbi_block((void*)dst_chunk, (const void*)src_chunk,
                               chunk_size, partner);

      process_chunk_warp_all_ready(lane, warp, warps, acc_chunk, dst_chunk,
                                   src_chunk, chunk_size, seq_num_local,
                                   POLL_TIMEOUT * 1000000000ULL);

      __syncwarp();
    }

    // The source pointer should be updated to the next step to the current
    // latest buffer
...
void RecursiveLL8Coll::initialize(int num_blocks, int threads_per_block,
                                  size_t chunk_size) {
  steps_ = 0;
...
void RecursiveLL8Coll::register_tensor(uint64_t id, size_t size,
...
template <typename T>
void RecursiveLL8Coll::allreduce_preallocated_impl(torch::Tensor& tensor,
                                                   uint64_t id,
                                                   cudaStream_t stream,
  ...
  Payload8B<T>* payload_scratch_send =
      (Payload8B<T>*)allocated_scratch_send_[id];
  Payload8B<T>* payload_scratch_recv =
      (Payload8B<T>*)allocated_scratch_recv_[id];
  size_t stride_size = stride_sizes_[id];
  uint32_t* seq_num = seq_nums_[id];
  uint64_t* seq_num_signal = seq_num_signals_[id];

  void* payload_args[] = {&payload_scratch_recv,
                          &payload_scratch_send,
                          &numel_reduced,
                          &steps_,
                          &steps_intra_,
                          &steps_inter_,
  ...
  dispatch_intra_reducescatter<T>((T*)src_sym, (const T*)src_sym, numel_reduced,
                                  stream);

  // Bump sequence number and wait on peers to have completed the last sequence
  // number
  bump_seq<<<1, 1, 0, stream>>>(seq_num, seq_num_signal, steps_inter_);

  // Launch kernel to properly pack data into Payload8B structures
  pack_payloads<T>(payload_scratch_send, (T*)src_sym, numel_reduced, seq_num,
                   stream);
  ...
  // TODO: Unpacking and final reduction can be done in a single kernel avoiding
  // the extra copy
  Payload8B<T>* final_payload = (Payload8B<T>*)((uint8_t*)payload_scratch_send +
                                                (steps_inter_ * stride_size));
  unpack_payloads<T>((T*)src_sym, final_payload, numel_reduced, seq_num,
                     seq_num_signal, steps_, steps_intra_, steps_inter_,
                     stream);

  // Intra-node all-gather
  dispatch_inter_allgather<T>((T*)src_sym, (const T*)src_sym, numel_reduced,
                              stream);
  return;
}
...
template <typename T>
void RecursiveLL8Coll::pack_payloads(Payload8B<T>* payloads, const T* data,
                                     size_t num_elements, uint32_t* seq_num,
                                     cudaStream_t stream) {
  const size_t num_cells =
      (num_elements + Payload8B<T>::N - 1) / Payload8B<T>::N;
  // TODO: Make this architecture specific
  const int block = 1024;
  const int grid = (int)((num_cells + block - 1) / block);
  pack_payloads_kernel<<<grid, block, 0, stream>>>(payloads, data, num_elements,
                                                   seq_num);
}

template <typename T>
void RecursiveLL8Coll::unpack_payloads(T* data, const Payload8B<T>* payloads,
                                       size_t num_elements, uint32_t* seq_num,
                                       uint64_t* seq_num_signal, int steps,
                                       int steps_intra, int steps_inter,
                                       cudaStream_t stream) {
  const size_t num_cells =
      (num_elements + Payload8B<T>::N - 1) / Payload8B<T>::N;
  const int block = 1024;
  const int grid = (int)((num_cells + block - 1) / block);
  unpack_payloads_kernel<<<grid, block, 0, stream>>>(
      data, payloads, num_elements, seq_num, seq_num_signal, steps, steps_intra,
      steps_inter);
}

#define INSTANTIATE_LL8_COLL_TYPE(T)                                       \
  template __global__ void pack_payloads_kernel<T>(                        \
      Payload8B<T>* __restrict__ payloads, const T* __restrict__ data,     \
      size_t num_elements, uint32_t* seq_num);                             \
  template __global__ void unpack_payloads_kernel<T>(                      \
      T* __restrict__ data, const Payload8B<T>* __restrict__ payloads,     \
      size_t num_elements, uint32_t* seq_num, uint64_t* signal, int steps, \
      int steps_intra, int steps_inter);                                   \
  template __global__ void recursive_allreduce_kernel_payload<T>(          \
      Payload8B<T> * dst, Payload8B<T> * src, size_t nreduce, int steps,   \
      int steps_intra, int steps_inter, size_t chunk_elems,                \
      size_t stride_size, uint32_t* seq_num, uint64_t* seq_num_signal);    \
  template void RecursiveLL8Coll::allreduce_preallocated_impl<T>(          \
      torch::Tensor & tensor, uint64_t id, cudaStream_t stream,            \
      const std::string& alg);                                             \
  template void RecursiveLL8Coll::pack_payloads<T>(                        \
      Payload8B<T> * payloads, const T* data, size_t num_elements,         \
      uint32_t* seq_num, cudaStream_t stream);                             \
  template void RecursiveLL8Coll::unpack_payloads<T>(                      \
      T * data, const Payload8B<T>* payloads, size_t num_elements,         \
      uint32_t* seq_num, uint64_t* signal, int steps, int steps_intra,     \
      int steps_inter, cudaStream_t stream);
