...
__global__ void recursive_allreduce_kernel_t(T* dst, T* src, size_t nreduce,
                                             int steps, int steps_intra,
                                             int steps_inter,
                                             uint64_t* chunk_signal,
  ...
  chunk_signal =
      chunk_signal + block_idx * steps_inter;  // one signal word per block
  ...
  for (int step = steps_intra; step < steps; ++step) {
    ...
    for (size_t chunk_idx = 0; chunk_idx < num_chunks; chunk_idx++) {
      ...
      T* scratch_chunk =
          (T*)dst + (inter_node_step + 1) * nreduce + chunk_idx * chunk_elems;

      nvshmemx_putmem_signal_nbi_block(
          (void*)scratch_chunk, (const void*)my_chunk, chunk_size * sizeof(T),
          chunk_signal + inter_node_step, 1, NVSHMEM_SIGNAL_ADD, partner);

      // wait for partner’s data to arrive
      if (thread_id == 0) {
        // The +1 is to account for the fact that the signal starts at 1, not 0
        nvshmem_signal_wait_until(chunk_signal + inter_node_step,
                                  NVSHMEM_CMP_GE, chunk_idx + 1);
        // printf("[%d, %d] step: %d, chunk_idx: %lu, signal: %lu\n", mype,
        // block_idx, step, chunk_idx, *(signal + step));
      }
      __syncthreads();
...
void RecursiveSimpleColl::initialize(int num_blocks, int threads_per_block,
                                     size_t chunk_size) {
  steps_ = 0;
...
void RecursiveSimpleColl::register_tensor(uint64_t id, size_t size,
  ...
  void* scratch =
      nvshmem_malloc((steps_inter_ + 1) * size_inter * torch::elementSize(dt));
  if (!scratch) {
    throw std::runtime_error("Failed to allocate scratch memory");
  }
  // Important to zero out the scratch memory
  cudaMemset(scratch, 0,
             (steps_inter_ + 1) * size_inter * torch::elementSize(dt));
...
void RecursiveSimpleColl::set_kernel_params(int num_blocks,
...
template <typename T>
void RecursiveSimpleColl::allreduce_preallocated_impl(torch::Tensor& tensor,
                                                      uint64_t id,
                                                      cudaStream_t stream,
  ...
  void* args[] = {&dst_sym,       &dst_sym,           &numel_reduced,
                  &steps_,        &steps_intra_,      &steps_inter_,
                  &chunk_signal_, &chunk_size_reduced};

  dispatch_intra_reducescatter((T*)dst_sym, (const T*)src_sym, numel_reduced,
                               stream);
  nvshmemx_collective_launch((const void*)recursive_allreduce_kernel_t<T>,
                             gridDim, blockDim, args, 0, stream);
  dispatch_intra_allgather((T*)src_sym,
                           (const T*)dst_sym + steps_inter_ * numel_reduced,
...
#define INSTANTIATE_SIMPLE_COLL_TYPE(T)                              \
  template void RecursiveSimpleColl::allreduce_preallocated_impl<T>( \
