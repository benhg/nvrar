...
#define CUDA_CHECK(stmt)                                      \
  do {                                                        \
    ...
    if (cudaSuccess != result) {                              \
      throw std::runtime_error(std::string("CUDA failed: ") + \
...
template <typename T>
__device__ inline T add_op(T a, T b) {
  return a + b;
}

template <>
__device__ inline __half add_op(__half a, __half b) {
  return __hadd(a, b);
}
...
template <typename T>
__device__ inline void reduce_add_vec(T* __restrict__ dst,
                                      const T* __restrict__ src, size_t n,
                                      int compute_threads, int compute_rank) {
...
__device__ inline void reduce_add_vec<__nv_bfloat16>(
  ...
  for (size_t i = size_t(compute_rank) * 2; i < vecN;
       i += size_t(compute_threads) * 2) {
